...
2. ### Запуск графического интерфейса (`pywebview`)
   * Выполните шаги из пункта выше (виртуальное окружение + `pip install -r requirements.txt`).
   * Убедитесь, что файл `sections.txt` существует в корне проекта (если его нет, создайте пустой файл):
     ```bash
     touch sections.txt
     ```
   * Запустите десктоп-режим:
     ```bash
     python desktop.py
     ```
   * Должно открыться окно `Sentilex`; при закрытии окна встроенный Flask-сервер останавливается автоматически.
   * Если Linux-система сообщает об отсутствии backend для `pywebview`, установите системные пакеты WebKit/GTK (пример для Debian/Ubuntu):
     ```bash
