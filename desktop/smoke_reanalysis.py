...
def run_once(input_path: Path, chunk_size: int) -> tuple[Path, Path, int, int]:
    filename = secure_filename(input_path.name)
    web_app.process_text(str(input_path), filename, chunk_size)

    result_txt = PROJECT_ROOT / "results" / f"{filename}_results.txt"
    result_png = PROJECT_ROOT / "results" / f"{filename}_sentiment_curve.png"
...
def main() -> int:
    ...
    txt_path, png_path, txt_mtime_1, png_mtime_1 = run_once(input_path, chunk_size=50)
    time.sleep(1.1)
    _, _, txt_mtime_2, png_mtime_2 = run_once(input_path, chunk_size=60)
    ...
    print("[smoke] OK: repeated analysis refreshed TXT and PNG outputs")
    return 0
